...
- **Never hand-write component CSS.** Buttons, tags, inputs, table cells, cards and nav
  items are already classes in `dist/components.css`, generated from Figma. Writing your
  own is how this project shipped a screen with perfect colours and invented shapes.
...
## Using the tokens and components

Two stylesheets. For a page:
...
both into a `<style>` block. Artifacts and canvases can't reference local files, so the
...
`dist/tokens.css` or `dist/components.css` — they are overwritten. Run `npm run check`
...
`npm run verify` runs six checks, each on a different axis. They are not
