...
// `scripts/lib/` was missing and the 2026-09-12 merge brought in docs that reference it —
// `rebind-rule.mjs` lives there and resolved nowhere.
const WHERE = p => ['', 'dist/', 'scripts/', 'scripts/lib/', 'tokens/_raw/', 'tokens/', 'docs/', 'assets/',
  'prototypes/', 'working/'];

// A FILE THE DOCS TELL YOU TO CREATE IS NOT A BROKEN REFERENCE — but it is only not one when it
// says so here. `figma-variables.json` is the Figma Variables API response: this environment cannot
// fetch it (api.figma.com is denied), CLAUDE.md documents the request a person runs to produce it,
// and `extract-tokens.mjs` exits 2 (vacuous) without it. Declared by name rather than by pattern,
// so the next missing file still fails.
const TO_BE_CREATED = new Set(['figma-variables.json']);
let refsChecked = 0;
for (const doc of DOC_FILES) {
  ...
  for (const ref of refs) {
    ...
    if (TO_BE_CREATED.has(ref)) continue;
    refsChecked++;
