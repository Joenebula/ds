...
export function backfill(text, nameCol, byName) {
  ...
  for (const line of lines.slice(1)) {
    ...
    if (id) filled++; else if (name) missing.push(name);
    out.push(cells.join('\t'));
  }
  return { text: out.join('\n') + '\n', filled, missing: [...new Set(missing)].sort() };
}
...
function main() {
  ...
  for (const c of inv) if (key(c.name) && c.nodeId) byName.set(key(c.name), c.nodeId);

  console.log(`${byName.size} name -> nodeId pairs available from components.json\n`);
  ...
  for (const { file, nameCol } of FILES) {
    ...
    console.log(`${file}`);
    console.log(`  ${r.filled} row-name(s) matched an inventory id, ${r.missing.length} did not`);
...
function selfTest() {
  ...
  console.log('self-test passed — the column is appended last and stays invisible to positional '
    ...
    + 'guess, and a second run neither duplicates the column nor overwrites a real id');
}
