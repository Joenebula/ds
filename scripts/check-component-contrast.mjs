...
const KNOWN = new Set([
  ...
  'pf-ag-field|light', 'pf-multi-select-checkbox|light', 'pf-toggle|light',
]);
...
const low = [];
let measured = 0, skipped = 0, translucent = 0;
for (const mode of ['light', 'dark']) {
  items.forEach((x, i) => {
    ...
    if (c < 4.5) low.push({ mode, x, c: +c.toFixed(2) });
  });
}
...
console.log(`  ${low.length} below AA, across ${new Set(low.map(l => l.x.base + '|' + l.mode)).size} `
  + `class/mode pair(s); ${KNOWN.size} are known and written up in docs/FIGMA-ISSUES.md §15`);
if (!measured) {
  console.error('  this check proved nothing: not one component painted its own background');
  process.exit(1);
}
for (const l of low) {
  const line = `.${l.x.base} [${l.x.variant}] in ${l.mode} mode reads ${l.c}:1 — its own text on `
    + `its own background`;
