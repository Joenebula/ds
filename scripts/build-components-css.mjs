...
function geometryDecls(g, notes, isVariant = false, composedType = null) {
  ...
  if (mode === 'HORIZONTAL' || mode === 'VERTICAL') {
    ...
    //
    // That is not hypothetical, and the measurement is worth stating exactly because a looser
    // reading of it was wrong. `.pf-layout-container-title` carried `gap: 464px`. Rendered at
    // 640px — the width scripts/screenshot-screen.mjs uses for a screen that declares no
    // viewport — working/case-mgmt-my-team overflowed horizontally to 972px; with the gap gone
    // it fits 640 exactly. That is the whole of the effect, and it is a real one: 332px of
    // horizontal overflow on a page every check called clean.
    //
    // What it is NOT: the same page at 1280px never overflowed and never wrapped, with the gap
    // or without it, because there the free space (506px) exceeds the gap and it does not bind.
    // The title wrapping to two lines at 640px is unchanged by this fix — that is a desktop
    // design in a mobile window, not a gap. Both were claimed here from a downscaled screenshot
    // and both were false; the numbers above come from measuring the before and after files.
    //
    // The same shape is `Selected action banner` at 899px, which verify-clipped DID catch, and
    // `Accordion` 689, `Layout container tabs` 204, `Metric card` 165 — 44 rows in all.
    //
...
// BOTH BRANCHES ADDED A RULE HERE AND NEITHER REPLACES THE OTHER: a focus ring, and a fix for a
// hug-contents table container that clipped its own last columns. Kept in that order.
// ---- focus ------------------------------------------------------------------
